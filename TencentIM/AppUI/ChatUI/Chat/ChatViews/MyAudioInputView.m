...
- (void)touchBegin:(NSSet*)touches
{
        ...
        [avSession requestRecordPermission:^(BOOL available) {
            ...
            else
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [HaloTool.sharedTool alterViewControllerWithTitle:@"无法录音" message:@"请在“设置-隐私-麦克风”中允许访问麦克风。" cancleTitle:@"确定" otherTitle:@"" otherSelectBlcok:^{}];
                });
            }
        }];
    }
    ...
    [UIView animateWithDuration:0.08f animations:^{
        self-> _recordBtn.transform = CGAffineTransformMakeScale(1.14, 1.14);
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:0.08 animations:^{
            self-> _recordBtn.transform = CGAffineTransformMakeScale(0.96, 0.96);
        } completion:^(BOOL finished) {
            [UIView animateWithDuration:0.04 animations:^{
                self-> _recordBtn.transform = CGAffineTransformIdentity;
            } completion:NULL];
        }];
    }];
}
