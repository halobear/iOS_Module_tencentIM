...
- (void)setupBottomToorBar
{
    ...
    if (kDeviceX) {
        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[toolBar(68)]-0-|" options:0 metrics:0 views:views]];
    }else{
        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[toolBar(49)]-0-|" options:0 metrics:0 views:views]];
    }
...
//    [layer setBorderColor:[[UIColor grayColor] CGColor]];
    [oriPicRadioBtn setImage:ImageNamed(@"too_uncheck") forState:UIControlStateNormal];
    [oriPicRadioBtn setImage:[UIImage imageNamed:@"too_selected"] forState:UIControlStateSelected];
...
- (UIButton *)createSendBtn
{
    ...
    sendBtn.frame = CGRectMake(0, 0, 70, 30);
    sendBtn.layer.cornerRadius = 3;
    [sendBtn addGradualColor:@[kColorRed,kColorPink] startPoint:CGPointMake(0, 0) endPoint:CGPointMake(1, 0)];
    sendBtn.enabled = YES;
    sendBtn.titleLabel.font = kAppMiddleTextFont;
    [sendBtn setTitle:@"发送" forState:UIControlStateNormal];
    [sendBtn setTitleColor:kTextColor_white forState:UIControlStateNormal];
