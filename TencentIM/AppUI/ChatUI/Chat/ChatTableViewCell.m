...
- (void)configContent
{
    ...
    _chatText.attributedText = [NSString getAttributedStringWithString:[elem text] lineSpace:3];
...
//        [data writeToFile:fileName atomically:YES];
        ...
        [HaloTool.sharedTool alterViewControllerWithTitle:@"提示" message:msg cancleTitle:@"确定" otherTitle:@"" otherSelectBlcok:^{ }];
    } fail:^(int code, NSString *msg) {
...
- (UIView *)addElemContent
{
    ...
    _cardMessageCell = [[TencentCardMessageCell alloc] initWithFrame:CGRectMake(0, 0, 236, 243)];
    return _cardMessageCell;
}
...
- (void)configContent
{
    ...
    NSLog(@"🍎=卡片消息===%@=",card);
    [_cardMessageCell.mImageView setProgressImageURL:[NSURL URLWithString:card[@"cover"]] WithPlaceholder:ImageNamed(img_placeholder) options:YYWebImageOptionProgressiveBlur completed:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {}];
    _cardMessageCell.mTitleLabel.text = card[@"title"];
    _cardMessageCell.mDesclabel.text = card[@"subtitle"];
    _cardMessageCell.mPriceLabel.text = [NSString stringWithFormat:@"%@",card[@"price"]];
    if (![_msg.msg isSelf]) {
        _cardMessageCell.mTitleLabel.textColor = UIColor.customBlack;
        _cardMessageCell.mDesclabel.textColor = UIColor.customBlack;
        _cardMessageCell.mPriceLabel.textColor = UIColor.customRed;
    }else{
        _cardMessageCell.mTitleLabel.textColor = UIColor.whiteColor;
        _cardMessageCell.mDesclabel.textColor = UIColor.whiteColor;
        _cardMessageCell.mPriceLabel.textColor = UIColor.whiteColor;
    }
    _cardMessageCell.cardClickBlock = ^{
        [TencentManger.sharedManger cardClickWithType:card[@"type"] detail_id:[NSString stringWithFormat:@"%@",card[@"id"]]];
    };
}
...
//==============================================================
@implementation ChatSystemTableViewCell

- (UIView *)addElemContent
{
    _systemMessageCell = [[TencentSystemMessageCell alloc] initWithFrame:CGRectMake(0, 0, kScreen_Width, 33)];
    return _systemMessageCell;
}
...
//    [super configContent];
    ...
    NSDictionary *card = [NSString parseJSONStringToNSDictionary:cardStr];
    NSLog(@"🍎=系统消息===%@=",card);
    _systemMessageCell.mTitleLabel.text = card[@"text"];
