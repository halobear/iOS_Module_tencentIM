...
+ (CGSize)getCellSize {
    UIImage *image = [UIImage MCImageNamed:@"non_normal"];
    return image.size;
}
- (void)dealloc {
...
- (void)layoutSubviews {
    ...
    if (![MaterialManager isOnlinePackage:_materialID]) {
        ...
        if ([SDImageCache.sharedImageCache diskImageDataExistsWithKey:[[SDWebImageManager sharedManager] cacheKeyForURL:iconURL]]) {
            [self startAnimation:NO];
        } else {
...
- (UIImageView *)icon {
    if (!_icon) {
        ...
        _icon.frame = CGRectMake(0, 0, [DynamicCell getCellSize].width, [DynamicCell getCellSize].height);
        _icon.bounds = self.bounds;
        _icon.layer.cornerRadius = [DynamicCell getCellSize].width * 0.5f;
...
- (UIImageView *)iconSelected {
    if (!_iconSelected) {
        ...
        _iconSelected.frame = CGRectMake(0, 0, image.size.width, image.size.height);
        _iconSelected.image = image;
        _iconSelected.alpha = 0.f;
        _iconSelected.layer.cornerRadius = image.size.width * 0.5f;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([collectionView isEqual:self.dynamicCollectionView]) {
        if (indexPath.item == 0) {
            ...
            DynamicCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[DynamicCell identifier] forIndexPath:indexPath];
            cell.materialID = [self.materials objectAtIndex:indexPath.item-3];
            cell.selected = self.selectedMaterialID.length > 0 && [cell.materialID isEqualToString:self.selectedMaterialID];
            [cell setNeedsLayout];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([collectionView isEqual:self.dynamicCollectionView]) {
        if (indexPath.item == 0) {
            ...
            NSString *materialID = [self.materials objectAtIndex:indexPath.item-3];
