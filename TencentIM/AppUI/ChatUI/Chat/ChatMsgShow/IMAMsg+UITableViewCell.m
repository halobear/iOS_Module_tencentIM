...
{
    if (_type == EIMAMSG_Custom) {
        TIMCustomElem *elem = (TIMCustomElem*)[_msg getElem:0];
        ...
        if ([card[@"type"] isEqualToString:@"goods_card"] || [card[@"type"] isEqualToString:@"hotel_card"]) {
            return [NSString stringWithFormat:@"IMAMsgCell1_%d", (int)_type];
        }
        else if ([card[@"type"] isEqualToString:@"system"]) {
            return [NSString stringWithFormat:@"IMAMsgCell2_%d", (int)_type];
        }
    }
...
-(Class)getCustomlass{
    ...
    if ([card[@"type"] isEqualToString:@"goods_card"] || [card[@"type"] isEqualToString:@"hotel_card"]) {
        ...
        return [ChatSystemTableViewCell class];
    }
    else{
        return [RichChatTableViewCell class];
    }
...
#pragma mark- 自定义消息height
    if (self.type == EIMAMSG_Custom) {
        TIMCustomElem *elem = (TIMCustomElem*)[_msg getElem:0];
        NSString *cardStr = [[NSString alloc] initWithData:elem.data encoding:NSUTF8StringEncoding];
        NSDictionary *card = [NSString parseJSONStringToNSDictionary:cardStr];
        if ([card[@"type"] isEqualToString:@"goods_card"] || [card[@"type"] isEqualToString:@"hotel_card"]) {
...
- (UIEdgeInsets)contentBackInset
{
    if (self.isMineMsg)
    {
        return UIEdgeInsetsMake(kDefaultMargin, kDefaultMargin, kDefaultMargin, kDefaultMargin);
    }
    else
    {
        return UIEdgeInsetsMake(kDefaultMargin, kDefaultMargin, kDefaultMargin, kDefaultMargin);
...
- (CGSize)contentBackSizeInWidth:(CGFloat)width
{
    ...
    if (self.type == EIMAMSG_Custom) {
        ...
        NSDictionary *card = [NSString parseJSONStringToNSDictionary:cardStr];
        if ([card[@"type"] isEqualToString:@"goods_card"] || [card[@"type"] isEqualToString:@"hotel_card"]) {
            CGSize size = CGSizeMake(215, 248);
            ...
            CGSize size = CGSizeMake(kScreen_Width, 33);
            UIEdgeInsets inset = [self contentBackInset];
            size.width += inset.left + inset.right;
            size.height += inset.top + inset.bottom;
            return size;
