...
- (void)onConnSucc
{
    ...
    [[QalSDKProxy sharedInstance] getSDKVer];
...
//        UIAlertView *alert = [UIAlertView bk_showAlertViewWithTitle:@"下线通知" message:@"您的帐号于另一台手机上登录。" cancelButtonTitle:@"退出" otherButtonTitles:@[@"重新登录"] handler:^(UIAlertView *alertView, NSInteger buttonIndex) {
        UIAlertView *alert = [UIAlertView bk_showAlertViewWithTitle:@"下线通知" message:@"您的帐号于另一台手机上登录。" cancelButtonTitle:@"知道了" otherButtonTitles:nil handler:^(UIAlertView *alertView, NSInteger buttonIndex) {
            if (buttonIndex == 0)
            {
                // 退出
                [self logout:^{
                    [[AppDelegate sharedAppDelegate] setLoginVC];
...
//                [[AppDelegate sharedAppDelegate] setLoginVC:false];
//            } fail:^(int code, NSString *msg) {
//                [[AppDelegate sharedAppDelegate] setLoginVC:false];
//            }];
//        }
