...
- (void)asyncSetFacename:(NSString *)faceName  succ:(TIMSucc)succ fail:(TIMFail)fail
{
    if ([NSString isEmpty:faceName])
    {
        //        [[HUDHelper sharedInstance] tipMessage:@"头像地址不能为空"];
        return;
    }
    ...
    option.userCustom = nil;
    TIMUserProfile *profile = [[TIMUserProfile alloc] init];
    profile.faceURL = faceName;
    
    [[TIMFriendshipManager sharedInstance] modifySelfProfile:option profile:profile succ:^{
        ws.profile.faceURL = faceName;
        if (succ)
        {
            succ();
        }
